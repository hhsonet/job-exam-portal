              ...
              <tr>
                ...
                <td>
                  ...
                  <?php elseif ($question['type'] === 'typing'): ?>
                    <div class="answer-box"><div><?= $isUnanswered ? '<span style="color:#a36b1b">Not answered</span>' : 'Answer hidden' ?></div><?php if (!$isUnanswered): ?><a download="typing-answer-submission-<?= (int) $submission['id'] ?>-question-<?= (int) $question['id'] ?>.pdf" href="<?= site_url('admin/submissions/' . $submission['id'] . '/question/' . $question['id'] . '/typing-pdf') ?>" style="display:inline-block;margin-top:10px;color:#168fd4;font-weight:700">Download answer PDF</a><?php endif; ?><div class="typing-verification"><strong>Similarity:</strong> <?= esc(rtrim(rtrim(number_format((float) ($question['similarity_percent'] ?? 0), 2, '.', ''), '0'), '.')) ?>% · <?= esc($question['verification_status'] ?? 'Not evaluated') ?></div></div>
